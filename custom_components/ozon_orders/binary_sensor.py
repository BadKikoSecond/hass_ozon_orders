"""Binary sensor platform for Ozon Orders."""
...
async def async_setup_entry(
    ...
    async_add_entities([OzonSessionBinarySensor(coordinator)])
    register_platform_add_entities(hass, entry, "binary", async_add_entities)


class OzonSessionBinarySensor(CoordinatorEntity[OzonOrdersCoordinator], BinarySensorEntity):
    """Whether Ozon cookies/session still work."""

    _attr_translation_key = "session_valid"
    _attr_device_class = BinarySensorDeviceClass.CONNECTIVITY
    _attr_icon = "mdi:cloud-check"

    def __init__(self, coordinator: OzonOrdersCoordinator) -> None:
...
class OzonOrderBinarySensor(CoordinatorEntity[OzonOrdersCoordinator], BinarySensorEntity):
