...
from .api.cookies import cookies_for_session, load_cookies, parse_cookies_input
...
STEP_RECONFIGURE_SCHEMA = vol.Schema({vol.Required(CONF_COOKIES): str})
...
async def _validate_connection(hass: HomeAssistant, cookies_raw: str) -> dict[str, Any]:
    await hass.async_add_executor_job(load_cookies, cookies_raw)
    cookies = await hass.async_add_executor_job(cookies_for_session, cookies_raw)
    ...
    async def async_step_user(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        return self.async_show_form(
            ...
            description_placeholders={
                "hint": (
                    "1) Откройте ozon.ru/my/orderlist в Chrome на ПК\n"
                    "2) Cookie-Editor → Export → JSON (весь массив .ozon.ru)\n"
                    "3) Вставьте сюда и подтвердите один раз"
                ),
            },
        )
    ...
    async def async_step_reconfigure(
        ...
        if user_input is not None:
            ...
            try:
                parsed = await self.hass.async_add_executor_job(parse_cookies_input, pasted)
                cookies_raw = json.dumps(parsed, ensure_ascii=False)
                await _validate_connection(self.hass, cookies_raw)
        ...
        return self.async_show_form(
            ...
            errors=errors,
            description_placeholders={
                "hint": "Новый JSON cookies после успешного входа на ozon.ru в браузере",
            },
        )
