#!/usr/bin/env python3
"""Probe Ozon API access from current Python environment (e.g. HA container)."""
...
API_URL = (
    ...
    f"?url={quote('/my/orderlist?selectedTab=active', safe='')}"
)
...
async def test_aiohttp(cookies: dict[str, str]) -> None:
    ...
    async with aiohttp.ClientSession(headers=HEADERS, cookie_jar=jar) as session:
        ...
        try:
            async with session.get(API_URL, allow_redirects=True, max_redirects=5) as resp:
...
async def test_curl_cffi(cookies: dict[str, str], impersonate: str) -> None:
    ...
    try:
        async with AsyncSession(impersonate=impersonate) as session:
            ...
            r2 = await session.get(API_URL, cookies=cookies, headers=HEADERS)
            ...
            if r2.status_code == 200 and "json" in (r2.headers.get("content-type") or ""):
                user = (r2.json().get("userInfo") or {}).get("user") or {}
                print("logged_in", user.get("isLoggedIn"), "name", user.get("firstName"))
            else:
