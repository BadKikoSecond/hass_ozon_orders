...
1. **Firefox** (рекомендуется) или Chrome — залогиньтесь на [ozon.ru](https://www.ozon.ru)
2. Откройте **`https://www.ozon.ru/my/orderlist`**, подождите **15–20 сек**
3. **Cookie-Editor** → Export → **JSON** (весь массив, в JSON должен быть **`abt_data`**)
4. **Настройки → Устройства и службы → Добавить → Ozon Orders**
5. Вставьте JSON → на **втором шаге** задайте интервал и antibot-параметры

### Шаг «Синхронизация и antibot» (мастер и настройки интеграции)

| Параметр | По умолчанию | Зачем |
|---|---|---|
| Интервал опроса | **30 мин** | Как часто обновлять список заказов |
| Детали заказов | вкл | Названия товаров (доп. запросы к Ozon) |
| Повтор деталей | **6 ч** | Не дёргать detail-страницы каждый цикл |
| Деталей за цикл | **1** | Порция detail-запросов за один опрос |
| Пауза между деталями | **2 сек** | Задержка между detail-запросами |
| Ручной refresh | **10 мин** | Лимит службы `ozon_orders.refresh` |

При **403** интеграция автоматически уходит в backoff (60→120→… мин).
...
Не чаще **одного раза в 10 минут** на аккаунт. Во время antibot-backoff служба игнорируется (см. атрибут `backoff_until` у `sensor.*_active_orders`).

---

## Интервал опроса

**Настройки → Устройства и службы → Ozon Orders → Настроить**

Все antibot-параметры (интервал, детали, backoff-поведение) — на этом экране. Старые установки без новых ключей получают значения по умолчанию из таблицы выше.
...
- Интеграция **бережёт лимиты**: одна HTTP-сессия на весь uptime, детали заказов — порциями, при 403 — экспоненциальный backoff (не долбит API каждые N минут).
...
- Не ставьте интервал опроса 15 мин «на всякий случай» и не жмите `refresh` подряд — так выше шанс antibot на IP Home Assistant.
